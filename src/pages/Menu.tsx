...
export default function Menu() {
  ...
  const handleNavigate = (filePath: string) => {
    ...
    navigate(`/editor/${encode}`);
  };

  const debouncedEventHandler = useMemo(() => debounce(handleNavigate, 300), []);
  ...
  useEffect(() => {
    ...
    const startFileDropHover = async () => {
      unlistenFileDrop = await listen("tauri://file-drop-hover", (event: any) => {
        setIsDrag(true);
      });
    };

    const startFileDropCancelled = async () => {
      unlistenFileDropCancelled = await listen("tauri://file-drop-cancelled", (event: any) => {
        setIsDrag(false);
      });
    };
  ...
  return (
    <div onClick={fileClick} className={`h-screen flex justify-center items-center cursor-pointer ${isDrag ? "bg-gray-900 blur duration-100 transition-all ease-out" : ""}`}>
      <h1 className="text-white text-2xl font-bold text-center">Drag and Drop Video to Compress</h1>
    </div>
  );
}
